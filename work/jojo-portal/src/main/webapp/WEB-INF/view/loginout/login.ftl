...
#inputs input
{
    background: #f1f1f1 url(${rc.contextPath}/images/login-sprite.png) no-repeat;
...
<body>
<form id="login" action="${rc.contextPath}/login" method="post">
	<div class="form-header">欢迎使用物料管理系统</div>
    <h1>登录系统</h1>

    <fieldset id="inputs">
        <label for="userId">登录名</label>
		<input id="userId" name="userId" type="text" value="${form.userId!}" placeholder="登录名" autofocus required/>

        <label for="password">密　码</label>
		<input id="password" name="pwd" type="password" value=""  placeholder="密码" required/>
    </fieldset>

    <fieldset id="actions">

       <input type="hidden" name="redirectURL" value="${form.redirectURL!}" />
		<input type="submit" id="submit" class="submit-btn" value="登　录">
		<#--
...
<!-- BSA AdPacks code -->
<script src="${rc.contextPath}/scripts/portal/adpacks-login.js"></script>
